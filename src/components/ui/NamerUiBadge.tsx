...
export default function NamerUiBadge({
  ...
  const applyHoverStyle = (el: HTMLElement | null) => {
    if (!el) return;
    Object.assign(el.style, { ...baseStyle, ...hoverStyle });
  };

  const applyBaseStyle = (el: HTMLElement | null) => {
    if (!el) return;
  ...
  const handleMouseEnter = () => {
    applyHoverStyle(buttonRef.current);
  };

  const handleMouseLeave = () => {
    applyBaseStyle(buttonRef.current);
  };
  ...
  return (
    <>
      ...
      >
        {/* Powered by - #a1a1aa normal, hsl(var(--background)) hover */}
        <span 
        ...
        >
          ...
          {/* Namer UI - CONSTANT hsl(var(--foreground)) */}
      ...
      <style jsx>{`
        ...
        /* Powered by hover color ONLY - specific class selector */
