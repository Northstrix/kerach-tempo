...
export default function ColorPicker({
  ...
  // âœ… FIXED: Track source of change + hex completely independent
  ...
  // âœ… FIXED: Manual hex sync ONLY for external changes
  ...
  const parseNumericChannel = (raw: string, min: number, max: number) => {
    ...
    if (!trimmed.length) return { value: 0, isValid: true }; // Treat empty as 0 and valid
    const n = Number(trimmed);
    if (Number.isNaN(n) || n < min || n > max) return { value: 0, isValid: false };
  ...
  // âœ… FIXED HEX - TRACKS SOURCE + NEVER AUTO-CORRECTS
  ...
  const handleRgbChannelChange = (channel: "r" | "g" | "b", raw: string) => {
    ...
    const parsedR = parseNumericChannel(newInputs.r, ranges.r[0], ranges.r[1]);
    const parsedG = parseNumericChannel(newInputs.g, ranges.g[0], ranges.g[1]);
    const parsedB = parseNumericChannel(newInputs.b, ranges.b[0], ranges.b[1]);
  
    setRgbErrors({ r: !parsedR.isValid, g: !parsedG.isValid, b: !parsedB.isValid });
  
    if (parsedR.isValid && parsedG.isValid && parsedB.isValid) {
      triggerChange(rgbaToHsva({ r: parsedR.value, g: parsedG.value, b: parsedB.value, a: 1 }), 'external');
    }
  };
  ...
  const handleHslChannelChange = (channel: "h" | "s" | "l", raw: string) => {
    ...
    const parsedH = parseNumericChannel(newInputs.h, ranges.h[0], ranges.h[1]);
    const parsedS = parseNumericChannel(newInputs.s, ranges.s[0], ranges.s[1]);
    const parsedL = parseNumericChannel(newInputs.l, ranges.l[0], ranges.l[1]);
  
    setHslErrors({ h: !parsedH.isValid, s: !parsedS.isValid, l: !parsedL.isValid });
  
    if (parsedH.isValid && parsedS.isValid && parsedL.isValid) {
      triggerChange(hslaToHsva({ h: parsedH.value, s: parsedS.value, l: parsedL.value, a: 1 }), 'external');
    }
  };
  ...
  // âœ… FIXED Saturation - external source
  ...
  // âœ… FIXED: Add autocomplete="off" to prevent browser interference
  return (
    <div className={cn("flex flex-col select-none", isRTL && "rtl")} style={{
      ...
      <CustomSlider
        ...
        // âœ… FIXED: All 3 static track props
...
type FloatingLabelInputProps = {
  ...
  height: number;
  bg: string;
  borderColor: string;      // â† IMMEDIATELY responsive
  borderWidth: number;
  radius: number;
  fontSize: number;
  textColor: string;
  focusBorderColor: string;
  floatingLabelBg: string;  // â† IMMEDIATELY responsive
  floatingLabelTextColor: string;
  floatingLabelActiveTextColor: string;
  floatingLabelRadius: number;
  floatingLabelBorderColor?: string;
...
export const FloatingLabelInput: React.FC<FloatingLabelInputProps> = ({
  ...
  // âœ… FIXED: IMMEDIATE parent prop reactivity
  ...
  // âœ… FIXED: useEffect for immediate parent prop changes
  React.useEffect(() => {
    if (inputRef.current) {
  ...
  // âœ… FIXED: Manual style updates respect parent changes instantly
  ...
  return (
    <div 
      ...
      style={{
        ...
        "--parent-background": floatingLabelBg,  // â† IMMEDIATELY updates
    ...
    >
      <input
        ...
        maxLength={maxLength}
        autoComplete="off"  // âœ… Browser autofill prevention
        inputMode="text"    // âœ… Better mobile
        spellCheck="false"  // âœ… No autocorrect
        style={{
          backgroundColor: bg,
          borderRadius: radius,
          borderStyle: "solid",
      ...
      <label 
        ...
        style={{
          color: labelTextColor,  // â† IMMEDIATELY responsive
          ...
          backgroundColor: floatingLabelBg,  // â† EXPLICIT background
        }}
      >
        {label}
      </label>
      ...
      <style jsx>{`
        ...
        .floating-input {
          ...
          transition: border-color 0.15s ease;  // âœ… Faster transition
          line-height: 1.4;
        }
...
export function CustomSlider({
  ...
  thumbHeight = '20px',
  width = '100%',
  trackBorderRadius = '8px',       // âœ… NEW DEFAULT
  trackBorderWidth = '0px',        // âœ… NEW DEFAULT (0px = no border)
  trackBorderColor = 'transparent', // âœ… NEW DEFAULT
    ...
    <div className="w-full">
      <style jsx>{`
        ...
        .${trackClass} {
          ...
          border-radius: ${trackBorderRadius} !important;           // âœ… NEW: Static rounding
          border: ${trackBorderWidth} solid ${trackBorderColor} !important;  // âœ… NEW: Static border (0px = none)
          z-index: 1;
        }
        .${rangeClass} {
          background: ${fillColor};
          border-radius: ${trackBorderRadius};                       // âœ… Matches track rounding
          ${isRTL ? 'right: 0;' : 'left: 0;'}
          width: ${percentage}%;
          z-index: 2;
        }
